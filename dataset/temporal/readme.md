...
### 2. Handle Missing Values (MICE)

**Where:** Stage 7 (`impute_missing_mice`)

The imputation follows a **three-layer strategy**:

| Data Type | Method |
|-----------|--------|
| **Static numerics** (age, weight_kg, bp_systolic, etc.) | `IterativeImputer` (MICE with BayesianRidge, 20 iterations, seed=42) |
| **Static categoricals** (sex, outcome, diagnosis, etc.) | Mode-fill, **but with a >90% missing guard** — if a column had >90% null, it gets `"Unknown"` instead of a misleading single-value mode |
| **Temporal numerics** (M0–M12 doses, weight, adherence, etc.) | **Step 1:** Forward-fill within each patient (`groupby('patient_id').ffill()`) — respects time order, no backward fill. **Step 2:** MICE on remaining gaps (includes `month` as a feature for temporal context) |
| **Temporal categoricals** | Forward-fill within patient, then mode or `"Unknown"` (same >90% guard) |

**Post-MICE sanity clipping:** After MICE runs, values are re-clipped to clinical bounds:
- Weight: [10, 180] kg
- Height: [80, 220] cm
- Monthly doses taken: [0, 31]
- Monthly missed doses: [0, ∞)
- Smear/LAMP and Xpert results: [0, 1]

MICE can extrapolate beyond plausible ranges, so this clipping is essential.
...
| 2 | Handle missing values (MICE) | **PASS** | 0 missing across all outputs |
...
| 6 | Schema validation | **PASS** | X_temporal (205,13,8), X_static (205,78), X_combined (205,182) |
