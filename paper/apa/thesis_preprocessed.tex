...
% placeins provides  to anchor floats within subsections.
\usepackage{placeins}
...
% placed closer to their call-out text instead of drifting to the end.
       % up to 90% of a page can be top floats
    % up to 80% can be bottom floats
      % at least 10% of a page must be text
 % float-only pages must be 80% full
              % allow up to 6 floats at the top
...
\includegraphics[width=\columnwidth]{tikz_1.png}%
\end{figure}
...
\includegraphics[width=\columnwidth]{tikz_2.png}%
\end{figure}
...
To prevent temporal data leakage, all missing value fills in the longitudinal structure are performed using forward-fill only, a patient's Month 4 observation can be carried forward to Month 5, but no future Month 6 measurement can inform an earlier time step that the model must predict without that knowledge. A monotonicity repair pass is applied after MICE imputation to correct any decreases in \texttt{cumulative\_doses\_taken} introduced by the multivariate imputer, ensuring that the cumulative dose variable remains non-decreasing for each patient across treatment months.

\subsubsection*{Categorical Harmonization Audit}

A cardinality audit of raw categorical features confirmed widespread data entry inconsistency: \texttt{chest\_x\_ray\_at\_case\_notification} contained over 300 unique free-text entries, \texttt{co\_morbidities} exceeded 150 variants, and nominally binary fields such as \texttt{sex} and \texttt{outcome} showed 5 to 12 unique raw representations due to inconsistent abbreviation conventions across treatment centers. These findings motivated a 17-field categorical harmonization pass that maps free-text and inconsistently coded entries into standardized categories before analysis.
...
\textbf{Accuracy} ($\frac{TP + TN}{TP + TN + FP + FN}$) measures the overall proportion of correct predictions. Although intuitive, accuracy is reported with the explicit caveat that it is dominated by the majority class under 86:14 imbalance.
...
\textbf{Specificity} ($\frac{TN}{TN + FP}$), the true negative rate, measures the proportion of actual successes correctly identified. High specificity ensures that patients on track are not unnecessarily flagged for review.

\textbf{F1 Score} ($2 \times \frac{Precision \times Recall}{Precision + Recall}$) is the harmonic mean of precision and recall, providing a single metric that penalizes extreme imbalance between the two. F1 is particularly informative when comparing models under different resampling strategies.

\textbf{Balanced Accuracy} ($\frac{Recall + Specificity}{2}$) averages the sensitivity and specificity, yielding a metric that is robust to class imbalance by construction. A balanced accuracy of 0.50 is equivalent to random guessing regardless of the class distribution.
...
\textbf{Area Under the Precision-Recall Curve (PR-AUC)} addresses this limitation by plotting precision against recall across thresholds, ignoring true negatives entirely. PR-AUC is therefore more informative than ROC-AUC when the positive class is rare \citep{davis2006relationship}. The no-skill baseline for PR-AUC equals the prevalence of the positive class (0.138), providing a natural reference point.
...
\path{risk\_factor\_s\_for\_drug\_resistance\_tuberculosis} & 599 & 100.0 & Yes \\
...
\path{regimen\_type\_at\_6th\_month\_of\_treatment} & 518 & 86.48 & Yes \\
...
\path{regimen\_type\_at\_end\_of\_treatment} & 355 & 59.27 & No \\
...
\path{name\_of\_treatment\_unit} & 290 & 48.41 & No \\
m0\_pct\_adherence & 277 & 46.24 & No \\
\path{treatment\_regimen} & 273 & 45.58 & No \\
...
\path{continuation\_phase\_start\_date} & 239 & 39.9 & No \\
m3\_monthly\_missed\_doses & 235 & 39.23 & No \\
m0\_height & 235 & 39.23 & No \\
\path{date\_of\_outcome} & 235 & 39.23 & No \\
m4\_monthly\_missed\_doses & 233 & 38.9 & No \\
m6\_monthly\_doses\_taken & 228 & 38.06 & No \\
\path{continuation\_phase\_end\_date} & 228 & 38.06 & No \\
m0\_monthly\_missed\_doses & 226 & 37.73 & No \\
m2\_monthly\_missed\_doses & 222 & 37.06 & No \\
\path{intensive\_phase\_end\_date} & 217 & 36.23 & No \\
m3\_cumulative\_doses\_taken & 217 & 36.23 & No \\
\path{drug\_resistance\_bacteriological\_status} & 217 & 36.23 & No \\
\path{chest\_x\_ray\_at\_case\_notification} & 213 & 35.56 & No \\
...
\path{date\_of\_notification} & 159 & 26.54 & No \\
m5\_cumulative\_doses\_taken & 157 & 26.21 & No \\
m4\_cumulative\_doses\_taken & 152 & 25.38 & No \\
\path{name\_of\_diagnosing\_facility} & 150 & 25.04 & No \\
\path{intensive\_phase\_start\_date} & 136 & 22.7 & No \\
outcome & 127 & 21.2 & No \\
m2\_cumulative\_doses\_taken & 126 & 21.04 & No \\
\path{regimen\_type\_at\_start\_of\_treatment} & 122 & 20.37 & No \\
\path{case\_registration\_group} & 117 & 19.53 & No \\
\path{date\_of\_diagnosis} & 110 & 18.36 & No \\
...
\path{bacteriologic\_status} & 79 & 13.19 & No \\
diagnosis & 62 & 10.35 & No \\
\path{treatment\_start\_date} & 57 & 9.52 & No \\
m2\_monthly\_doses\_taken & 57 & 9.52 & No \\
m1\_monthly\_doses\_taken & 39 & 6.51 & No \\
sex & 33 & 5.51 & No \\
no & 31 & 5.18 & No \\
m0\_monthly\_doses\_taken & 30 & 5.01 & No \\
\path{date\_of\_birth} & 20 & 3.34 & No \\
age & 18 & 3.01 & No \\
\path{data\_year} & 0 & 0.0 & No \\
\path{source\_file} & 0 & 0.0 & No \\
facility & 0 & 0.0 & No \\
\end{longtable}
...
\begin{longtable}[l]{>{\raggedright\arraybackslash}p{0.19\textwidth}cccc}
...
\includegraphics[width=\columnwidth]{temporal/figures/temporal_persistence_cascade.png}
\end{figure}
...
\begin{figure}[t]
    ...
    \includegraphics[width=\columnwidth]{figures/yearly_case_counts.png}
\end{figure}

\begin{figure}[t]
    \caption{Age-Sex Pyramid of the Study Cohort}
    \label{fig:age_sex_pyramid}
    \includegraphics[width=\columnwidth]{figures/age_sex_pyramid.png}
\end{figure}
...
\begin{figure}[t]
    ...
    \includegraphics[width=0.7\textwidth]{figures/treatment_outcomes_pie.png}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.7\columnwidth]{figures/numerical_pearson_correlation.png}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[width=\columnwidth]{figures/categorical_cramersv_correlation.png}
\end{figure}
...
\textbf{XGBoost} & \textbf{SMOTE - ENN} & \textbf{0.6873} & \textbf{0.6904} & \textbf{0.0119} & \textbf{0.5501} & \textbf{0.1753} & \textbf{0.1971} & \textbf{0.7330} & \textbf{0.3106} & \textbf{0.9240} & \textbf{0.5207} & \textbf{0.6660} & \textbf{0.7056} \\
LightGBM & SMOTE - ENN & 0.6830 & 0.6898 & 0.0148 & 0.7941 & 0.1996 & 0.2939 & 0.3484 & 0.3188 & 0.8922 & 0.8656 & 0.8787 & 0.5492 \\
\bottomrule
\end{tabular}%
\smallskip

{\footnotesize \textit{Note.} F~=~Failure class; S~=~Success class; AUC~=~test-set ROC-AUC; CV-AUC~=~5-fold CV mean; $\pm$Std~=~CV standard deviation; Acc.~=~Accuracy; MCC~=~Matthews Correlation Coefficient; Prec.~=~Precision; Score~=~composite $0.6 \times \text{AUC} + 0.4 \times \text{Recall\,(F)}$. Bold row indicates recommended model.}
\end{table}
...
\begin{figure}[htbp]
  \includegraphics[width=0.95\textwidth]{figures/fig_roc_pr_curves.png}
...
\begin{figure}[htbp]
  \includegraphics[width=0.95\textwidth]{figures/fig_cv_feature_importance.png}
...
\includegraphics[width=\columnwidth]{temporal/figures/vitals_pearson_correlation.png}
\end{figure}
...
\includegraphics[width=\columnwidth]{temporal/figures/categorical_cardinality_audit.png}
\end{figure}
...
\includegraphics[width=\columnwidth]{temporal/figures/categorical_cramersv_correlation.png}
\end{figure}
...
\includegraphics[width=\columnwidth]{temporal/figures/categorical_cramersv_imputed.png}
\end{figure}
...
\includegraphics[width=\columnwidth]{temporal/figures/temporal_feature_correlation.png}
\end{figure}
...
\includegraphics[width=\columnwidth]{temporal/figures/temporal_lag_heatmap.png}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.85\textwidth]{figures/agent/patient_overview_tab.png}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.85\textwidth]{figures/agent/risk_analysis.png}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.85\textwidth]{figures/agent/full_breakdown.png}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.85\textwidth]{figures/agent/first_step.png}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.85\textwidth]{figures/agent/med_gemma.png} 
\end{figure}
...
\subsection*{B.1 Authentication and Dashboard}

\begin{figure}[H]
    \centering
    \caption{CDSS login screen.}
    \label{fig:app_login}
    \includegraphics[width=0.85\textwidth]{figures/agent/login.png}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.85\textwidth]{figures/agent/dashboard.png}
\end{figure}

\subsection*{B.2 Patient Intake Workflow}
...
\begin{figure}[H]
    ...
    \label{fig:app_step3}
    \includegraphics[width=0.85\textwidth]{figures/agent/third_step.png}
\end{figure}
...
\begin{figure}[H]
    ...
    \label{fig:app_step4}
    \includegraphics[width=0.85\textwidth]{figures/agent/fourth_step.png}
\end{figure}
...
\begin{figure}[H]
    ...
    \label{fig:app_step5}
    \includegraphics[width=0.85\textwidth]{figures/agent/fifth_step.png}
\end{figure}

\subsection*{B.3 Patient Profile and Records}
...
\begin{figure}[H]
    ...
    \label{fig:app_chart}
    \includegraphics[width=0.85\textwidth]{figures/agent/patient_chart.png}
\end{figure}

\subsection*{B.4 Longitudinal Monitoring}

\begin{figure}[H]
    \centering
    \caption{Monthly check-in log for adherence and follow-up imaging.}
    \label{fig:app_monthly}
    \includegraphics[width=0.85\textwidth]{figures/agent/log_monthly.png}
\end{figure}

\subsection*{B.5 Risk Prediction and Explainability}
