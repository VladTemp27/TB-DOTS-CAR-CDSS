...
To evaluate the impact of the preprocessing pipeline on categorical association structure, a second Cram\'{e}r's V matrix was computed from the cleaned dataset for comparison with the raw matrix. Both matrices were restricted to columns that satisfied the quality thresholds so that only reliable associations are reported. Three columns present in the raw matrix were absent from the cleaned matrix: \texttt{prior\_history\_of\_tb} and \texttt{dat\_supported} were excluded because their approximately 90\% missing rate left too few complete observation pairs for a meaningful association estimate, while \texttt{nationality} was excluded because harmonization collapsed all patient entries to a single value (Filipino), eliminating the categorical variance required by Cram\'{e}r's V. This comparison isolates the effect of categorical harmonization and imputation on the measured associations.
