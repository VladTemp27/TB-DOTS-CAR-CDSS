...
Figure~\ref{fig:roc_curves} presents the Receiver Operating Characteristic (ROC) curves
for all six classifiers under the best-performing sampling configuration. The area under
the curve (AUC) summarizes each model's ability to discriminate between treatment success
and failure across decision thresholds.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/fig_roc_curves.pdf}
  \caption{ROC curves for all classifiers under the best sampling strategy. AUC values
           are shown in the legend. The dashed diagonal represents random chance.}
  \label{fig:roc_curves}
\end{figure}
\FloatBarrier

Figure~\ref{fig:pr_curves} shows Precision-Recall curves, which are more informative
than ROC curves for the imbalanced TB outcome dataset (approximately 88\% success rate).

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/fig_pr_curves.pdf}
  \caption{Precision-Recall curves for all classifiers. The horizontal dashed line
           represents the no-skill baseline (class prevalence).}
  \label{fig:pr_curves}
\end{figure}
\FloatBarrier

Figure~\ref{fig:confusion_matrix} presents the confusion matrix for the best-performing
model evaluated on the held-out 10\% validation set.
...
\begin{figure}[htbp]
  ...
  \caption{Confusion matrix of the best model on the test set.
           Row-normalized percentages are shown in parentheses.}
  \label{fig:confusion_matrix}
\end{figure}
\FloatBarrier

Figure~\ref{fig:cv_boxplot} illustrates the distribution of 5-fold stratified
cross-validation ROC-AUC scores across all classifiers, indicating the stability
and generalizability of each model.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/fig_cv_boxplot.pdf}
  \caption{Distribution of 5-fold stratified cross-validation ROC-AUC scores per
           classifier. Higher median and lower spread indicate more stable performance.}
  \label{fig:cv_boxplot}
\end{figure}
\FloatBarrier

Figure~\ref{fig:feature_importance} displays the top 15 features ranked by importance
from the best-performing tree-based model, providing interpretability for clinical use.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{figures/fig_feature_importance.pdf}
  \caption{Top 15 feature importances from the best-performing classifier.
           Importance scores are normalized to sum to 1.}
