...
For the non-temporal track, a systematic model selection process first eliminated Logistic Regression (CV-AUC $\approx 0.63$; 80\% false-positive alert rate), SVM (CV-AUC $\approx 0.59$; near-random discrimination), and Gradient Boosting ($< 5\%$ failure recall in testing) as clinically unsuitable. The three retained ensemble tree-based classifiers were evaluated with two sampling strategies. Model selection applied a composite score $S = 0.6 \times \text{AUC} + 0.4 \times \text{Recall\,(Failure)}$, reflecting the asymmetric cost of missing a failure in a clinical alert system. XGBoost with SMOTE-ENN achieved the highest composite score ($S = 0.699$) by capturing 72.9\% of failure cases, and was selected as the recommended model. XGBoost without resampling achieved the highest AUC (0.712) and is recommended as the secondary model for continuous risk score output.
