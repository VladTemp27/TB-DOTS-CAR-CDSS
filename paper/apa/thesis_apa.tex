...
Pulmonary tuberculosis remains stubborn in the Cordilleras. Patients are not uniform. Coverage from the treatment program is not uniform either. Together those two facts keep producing treatment failures the system catches only in hindsight. This study trains and tests machine learning models that try to predict, ahead of time, which TB-DOTS patients are heading for that outcome. The data covers DOH-CAR facilities from 2015 through 2025. Before any model touched it, the records went through a preprocessing pipeline that normalizes schemas, pseudonymizes patients, and handles missing values diagnostically rather than with a single blanket rule. The diagnostic part is the bit worth flagging: instead of imputing everything the same way, the pipeline first asks how each variable went missing (MCAR, MAR, or MNAR) and routes it accordingly, to column exclusion, to listwise deletion in narrow cases, to a missing-indicator-with-fill, or to stochastic MICE when the assumptions actually hold. On the modeling side we use ensemble tree classifiers (XGBoost, LightGBM, Random Forest) for the cross-sectional data and Bi-LSTM architectures for the monthly monitoring data, since treatment runs across six to twelve months and the static models miss what changes during it. An accurate score by itself is not enough for clinical use, so Shapley Additive Explanations sit on top of the predictions and show which patient-level factors moved each estimate. The whole stack lives inside a Clinical Decision Support System that displays dynamic risk, SHAP-grounded explanations, and a bounded narrative layer, with the clinician still in the loop for the final call.
...
Past work on TB treatment outcomes has tended to focus on sociodemographic characteristics, comorbidities, medical history, and other patient-level risk factors. The findings have been mixed, and a fair amount of that inconsistency comes from methodology rather than from the disease being inherently unreadable. Most published analyses lean on traditional statistical techniques: hypothesis tests, regression models, factor-by-factor scrutiny, often with an implicit assumption that relationships among the variables are linear . That approach delivers real evidence on individual risk factors, but it is poorly equipped for what is happening underneath, which is several variables interacting at once in ways that do not separate cleanly.

Machine learning has an advantage there. An algorithm that learns from high-dimensional data can pick up the kinds of subtle dependencies that ultimately decide whether treatment holds together or breaks down. The variables involved in TB treatment, demographic, clinical, diagnostic, and treatment-related, interact in non-linear ways that a single-factor model has no real chance of capturing. A range of algorithms have been tried at this problem: logistic regression, Support Vector Machines, and ensemble methods like XGBoost, among others . Within TB specifically, earlier studies have applied ML to predicting drug resistance, flagging high-risk patients, and informing treatment strategy decisions . The throughline across that body of work is fairly simple. A predictive model can extend the reach of risk-factor analysis by reading multiple dimensions of patient data simultaneously, rather than treating them in isolation.

But a model that only outputs a number is not particularly useful in a clinical setting, no matter how accurate it is. A clinical decision support system needs to be interpretable. Without insight into why a patient was flagged as high-risk, a healthcare worker has nothing concrete to verify against their own observations and very little to act on. To address that, this study uses Shapley Additive Explanations (SHAP), a post-hoc method grounded in Shapley values that quantifies how much each variable contributed to a given prediction and that rolls up cleanly into both patient-level explanations and overall feature importance . SHAP fits especially well with tree-based clinical models such as XGBoost, Random Forest, and LightGBM, where accuracy is often high but the internal decision path is difficult to inspect directly. In the healthcare ML literature, SHAP is among the more widely used explanation methods, particularly on tabular data , and it has been used to surface real contributors to treatment failure in clinical ML work, including drug resistance, TB history, and lab markers .

Where fragmented, single-factor risk studies seem to run out of road, interpretable ML offers a way forward. Understanding what actually drives TB outcomes requires combining several dimensions of patient data rather than examining them one at a time, and this study is built on that idea. The framework spans demographic, clinical, diagnostic, and treatment-related variables. Descriptive analysis lays out patient profiles and treatment patterns. Supervised models estimate outcome risk on top of that. SHAP explanations then show which factors moved each estimate. The intent is to keep predictive accuracy high while keeping the output legible to providers , so that clinicians can see not just who is at higher risk of failure, but why the model thinks so, and where it might make sense to step in earlier with closer monitoring or modified case management.
...
The records that feed this study come from DOH-CAR's central holdings of PTB patient data for the Cordillera Administrative Region, covering 2015 through 2025. Work only began after ethical clearance came through, since the data is sensitive and the program had to sign off on use. The first thing we did was pseudonymize. Once identifiers were stripped, the data ran through a preprocessing pipeline that does the usual hygiene work, schema harmonization, error correction, and feature scaling, and then through the diagnostic-first missing data protocol described later in its own subsection. Before any predictive modeling, exploratory and cluster analyses describe the cohort itself and produce subgroup-level features. From there, three ensemble tree-based classifiers, XGBoost, LightGBM, and Random Forest, learn to predict treatment outcomes defined by National Tuberculosis Program criteria. TB treatment is not a single visit. It runs across months, and the monitoring data changes month to month, so the static classifiers are tested alongside temporal models, specifically Recurrent Neural Networks (RNNs) and Long Short-Term Memory (LSTM) networks, and the two families are ensembled for robustness. Standard classification metrics evaluate performance, while Shapley Additive Explanations (SHAP) handle interpretability. At the end, the validated models feed a clinical decision support framework that updates each patient's risk estimate as new monthly data arrives. Figure~\ref{fig:data_management} sketches the overall flow.
...
The temporal monitoring dataset received the same treatment before any preprocessing ran: a global missingness audit across every feature, with columns ranked by missing rate. Anything above 80\% missingness was excluded outright. Anything between 50\% and 80\% went into the missing-indicator-with-fill pathway, with the call weighed against the variable's predictive importance. This kind of threshold-based triage is fairly standard practice. The advantage is that the handling strategy adapts to the severity of the missingness rather than forcing one rule across the entire dataset .
...
We split the data at the patient level with a stratified 70/20/10 partition, so the training, test, and holdout validation sets each preserve the class proportions of the full cohort . The class imbalance is heavy on the order of 86/14, success against failure, and we correct it on the training set only, with one of three oversampling approaches. SMOTE  interpolates synthetic minority-class points between existing ones. SMOTE-ENN  layers Edited Nearest Neighbors undersampling on top of SMOTE to clear ambiguous borderline cases. SMOTE-Tomek  pairs SMOTE with Tomek link removal to clean up the decision boundary. Neither the test set nor the validation set ever gets resampled. They keep their original imbalanced distributions, which is the only way to keep the evaluation honest .
