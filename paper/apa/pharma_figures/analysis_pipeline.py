...
class TBAnalysisPipeline:
    ...
    def __init__(self, output_dir: Path | None = None) -> None:
        # This script lives in paper/apa/pharma_figures/, so the project root
        # is three levels up.
        base_dir = Path(__file__).resolve().parents[3]
        pharma_dir = base_dir / "paper" / "apa" / "pharma_figures"
        self.output_dir = output_dir or pharma_dir
        self.output_dir.mkdir(parents=True, exist_ok=True)

        # Figures, tables, and statistics all land in pharma_figures/.
        self.table_dir = self.output_dir
    ...
    def build_summary_table(self, df: pd.DataFrame) -> pd.DataFrame:
        summary = {
            ...
            "Clinically Diagnosed": df["Bacteriologic Status"].str.contains("Clinically", na=False).sum() if "Bacteriologic Status" in df.columns else "N/A",
        ...
        # --- Age group distribution (WHO-aligned life stages) ---
        if "Age" in df.columns:
            age = pd.to_numeric(df["Age"], errors="coerce")
            age_bins = [0, 10, 20, 60, 200]
            age_labels = [
                "Age Group: Children (0-9)",
                "Age Group: Adolescents (10-19)",
                "Age Group: Adults (20-59)",
                "Age Group: Older Adults (60+)",
            ]
            grp = pd.cut(age, bins=age_bins, right=False, labels=age_labels)
            counts = grp.value_counts()
            for label in age_labels:
                summary[label] = int(counts.get(label, 0))

        # --- Registration Group distribution (exact counts) ---
        reg_labels = {
            "NEW": "Reg. Group: New",
            "RELAPSE": "Reg. Group: Relapse",
            "TX AFTER LOST TO FF-UP": "Reg. Group: Treatment After Lost to Follow-up",
            "TX AFTER FAILURE": "Reg. Group: Treatment After Failure",
            "PREVIOUS TX OUTCOME UNKNOWN": "Reg. Group: Previous Treatment Outcome Unknown",
            "OTHER": "Reg. Group: Other",
        }
        if "Registration Group" in df.columns:
            counts = df["Registration Group"].value_counts()
            for raw, label in reg_labels.items():
                summary[label] = int(counts.get(raw, 0))
