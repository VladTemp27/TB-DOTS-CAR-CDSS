                  ...
                  <tbody>
                    {patient.monthlyRecords.map((record, i) => (
                      <tr key={record.timestamp} className={`border-b border-border/50 ${i % 2 === 0 ? '' : 'bg-gray-50/50 print:bg-gray-100'}`}>
                        ...
                        <td className="py-2 text-ink-secondary tabular-nums">{record.pctAdherence != null ? `${((record.pctAdherence > 1 ? record.pctAdherence : record.pctAdherence * 100)).toFixed(2)}%` : '—'}</td>
